<section>
    <div class="glass-card rounded-2xl p-6 mb-6">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-text-black dark:text-text-white">{{ __('Rarity Details') }}</h2>
            <div class="flex items-center gap-2">

                <x-ui.button href="{{ route('admin.gm.rarity.edit', encrypt($data->id)) }}" variant="secondary"
                    class="w-auto py-2!">
                    <flux:icon name="pencil"
                        class="w-4 h-4 stroke-text-btn-secondary group-hover:stroke-text-btn-primary" />
                    {{ __('Edit') }}
                </x-ui.button>
                <x-ui.button href="{{ route('admin.gm.rarity.index') }}" type='accent' class="w-auto! py-2!">
            ...
            <div class="mb-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div class="bg-slate-50 dark:bg-gray-700 rounded-lg p-4 border border-slate-200">
                        ...
                        @if ($data->icon)
                            <img src="{{ storage_url($data->icon) }}" alt=""
                    ...
                    <div class="bg-slate-50 dark:bg-gray-700 rounded-lg p-4 border border-slate-200">
                        ...
                        <p class="rouneded-3xl font-bold {{ $data->status->color() }}">
                            {{ $data->status->label() }}</p>
                    </div>
                    <div class="bg-slate-50 dark:bg-gray-700 rounded-lg p-4 border border-slate-200">
